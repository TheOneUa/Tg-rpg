...
function generateDungeon(depth) {
    ...
    for(let i=0; i<numItems && i<spawnCells.length; i++) {
        ...
        items.push(new Item(cell.x * CFG.TILE + CFG.TILE/2, cell.y * CFG.TILE + CFG.TILE/2, type));
        // Координаты в пикселях для Item
        items[items.length-1].x = cell.x;
        items[items.length-1].y = cell.y;
    }
